			...
			<div id="content">
				...
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							<li><a href="#">Home</a></li>
							<li class="active">Phrases List</li>
						</ol>
					...
					<div class="section-body">
						...
						<div class="card">
							<div class="card-body">
								...
								<form action="<?php echo base_url(); ?>admin_master/phrases_import" method="post" name="upload_excel" enctype="multipart/form-data" onsubmit="return checkvalid();">
									<div class="form-group row">
										...
										<div class="col-md-1 col-sm-3 col-xs-5 text-right mrgt5 pdr0">
											<a href="<?php echo base_url();?>admin_master/download_phrase_sample" class="btn btn-primary">Sample Excel File</a>
										</div>
									</div>
								</form>
								...
								<!-- BEGIN DATATABLE 2 -->
								<form action="<?php echo base_url(); ?>admin_master/delete_all_phrases" method="post" onsubmit="return confirm_delete();">
									<div class="row">
										<h4 class="col-md-12 col-sm-12 mrgt15">Phrase List</h4>
										...
										<div class="col-md-10 col-sm-9 mrgt10 pull-right">
											<a href="<?php echo base_url(); ?>admin_master/add_phrases" class="btn btn-primary btn-sm btn-raised pull-right">Add New</a>
											...
											<div class="table-responsive no-margin">
												...
												<table id="example" class="table order-column hover">
													...
													<tbody >
														...
														<tr>
															<td><label class="checkbox-inline checkbox-styled"><input type="checkbox" name="delete[]" class="checkBoxClass" value="<?php echo $key['phrases_id']; ?>"/><span></span></label></td>
															<td><?php echo $ctn; ?></td>
															
															<?php foreach($source_lang as $langkey){ ?>
															<td><?php echo ucfirst($key['phrase_'.$langkey['language_code']]); ?>
															...
															<td><a href="<?php echo base_url();?>admin_master/edit_phrases/<?php echo $key['phrases_id']; ?>"><i class="fa fa-pencil action"></i> </a>  <a onclick="return confirm('Are you sure?');" href="<?php echo base_url();?>admin_master/delete_phrases/<?php echo $key['phrases_id']; ?>"> <i class="fa fa-trash-o action"></i> </a></td>
														</tr>
														<?php $ctn++; } ?>
													</tbody>
...
<script type="text/javascript">
	...
	//var sort= $('#sort').val();
			...
			$.ajax({
					...
					success:function(data){
						...
						$().redirect('<?php echo base_url();?>admin_master/phrases_list', {'cate_id':cate_id,'subcate_id':subcate});
					},			
			}); 
	...
	//var sort= $('#sort').val();
		...
		$.ajax({
					...
					url:'<?php echo base_url();?>admin_master/phrases_list/'+mode+'/'+category+'/'+subcate,
					success:function(data){
						
						//window.location.href = '<?php echo base_url();?>admin_master/words_list/'+mode+'/'+category+'/'+subcate;
						$().redirect('<?php echo base_url();?>admin_master/phrases_list', {'cate_id':category,'subcate_id':subcate});
	...
	//var sort= $('#sort').val();
			...
			$.ajax({
					url:'<?php echo base_url();?>admin_master/phrases_list/',
					...
					//	$('#subcate').find('option').remove().end().append(data);
						//window.location.href = '<?php echo base_url();?>admin_master/words_list/'+mode+'/'+cate_id+'/'+subcate;
						$().redirect('<?php echo base_url();?>admin_master/phrases_list', {'mode_id': mode,'cate_id':cate_id,'subcate_id':subcate,'per_page':per_page});
					},			
			}); 
